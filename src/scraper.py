...
def main(dev, test):
    ...
    for mrm in only_files:
        # if mrm not in ['MentalHealthAddictionRecovery.csv', ]:
        #     continue
        name = "./data/" + mrm
        print(mrm)
        with open(name, 'r') as file:
            ...
            if DEV:
                # Limit length of data to 5 for testing
                paa = {k: paa[k] for k in list(paa)[:5]}

            for k in list(paa.keys()):

                name_of_file = "{0}_{1}_{2}_{3}.txt".format(paa[str(k)]["Organization Name"],
                                                            paa[str(k)]["Topic area"], paa[str(k)]["type"], str(k))
                ...
                if str(paa[str(k)]["URL"]).__contains__('.pdf'):
                    ...
                    try:
                        ...
                        pdf_file_obj = open(final_file_name, 'rb')
...
def openfolder(namer, paa, k):
    ...
    if not os.path.exists(namer + "/" + paa[str(k)]["Organization Name"].replace(" ", "_")):
        try:
            print("Creating Folder: ", paa[str(k)]["Organization Name"].replace(" ", "_"))
    ...
    if not os.path.exists(
            namer + "/" + paa[str(k)]["Organization Name"].replace(" ", "_") + "/" + paa[str(k)][
                "type"].replace(" ", "_")):
        try:
            print("Creating Folder: ", paa[str(k)]["type"].replace(" ", "_"))
            os.makedirs(
                namer + "/" + paa[str(k)]["Organization Name"].replace(" ", "_") + "/" + paa[str(k)][
                    "type"].replace(" ", "_"))
        except Exception as e:
